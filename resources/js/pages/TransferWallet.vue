    ...
    <div class="py-4">
        ...
        <div class="px-4">
            <div class="before:rounded-xl border_grad2 mt-2 mb-5">
                <input v-model="amount" type="text" :placeholder="__('Enter the transfer amount')" class="py-7 w-full bg-transparent focus:outline-none text-center text-4xl font-bold text-white placeholder:text-lg">
            </div>
            <div class="px-5 text-white text-sm py-5">
                {{__('Here are the instructions for transferring balances to wallet')}}:
                <div class="">
                    <div class="text-sm text-gray-500 my-3">{{__('1')}}. {{__('Money generated by recycling box products can be used for cash withdrawl')}}.</div>
                    <div class="text-sm text-gray-500 my-3">{{__('2')}}. {{__('The recharge amount cannot be directly used for withdrawal')}}.</div>
                </div>
            </div>
        </div>
        <div class="absolute w-full bottom-5 px-4">
            <div class="py-3 btn_gradient rounded-full w-full text-white text-center" @click="submit()">{{__('Submit')}}</div>
        </div>
