<template>
     <div v-show="modelActive" class="boxModal">
        <div class="boxModal-content">
            ...
            <div class="text-center px-5 -mt-5">{{__('You have won special prize')}}<span v-for="(prize,j) in prizes" :key="j"> {{prize.itemName}}<span v-if="j == prizes.length-1"></span><span v-else>,</span></span>.</div>
                    ...
                    <div v-for="(prize,i) in prizes" :key="i" class="congratz_item_card">
                        ...
                        <label class="form-check-label" :for="'itemChecked'+i">
                            ...
                            <div class="congratz_item_card_footer text-center">
                                ...
                                <div class="pb-1" style="font-size:9px;">({{prize.price}} {{__('Ks')}})</div>
                            </div>
                        </label>   
                    </div>
                </div>
            ...
            <div class="flex justify-around">
                <div class=""><button class="btn_two rounded-full py-2 w-32 m-2 text-white" @click="sell()">{{__('Sell')}}</button></div>
                <div class=""><button class="btn_one rounded-full py-2 w-32 m-2 text-white" @click="collect()">{{__('Collect')}}</button></div>
            </div>
            <div class="flex justify-center mt-4 text-center text-sm" style="color:#8064E1;">
                <div class="self-center mr-3">
                    <img :src="$asset+'/image/ui/Share.svg'">
                </div>
                <div>{{__('Share your friends')}}</div>
            </div>
        </div>
    </div>
</template>
